...
class DeepDream:
    ...
    def __init__(self):
        if not p.exists(DeepDream._MODEL_FILENAME):
...
if __name__ == '__main__':
    ...
    process_frame = DeepDream().process_frame if True else process_frame_simple
    data_dir = p.expanduser('~/videos')
    run(p.join(data_dir, 'inputs/test.mp4'), p.join(data_dir, 'outputs/video.mp4'), process_frame)
